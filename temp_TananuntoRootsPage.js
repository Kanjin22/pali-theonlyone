...
function TananuntoRootsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Use a versioned key to force refresh when logic changes
            const CACHE_KEY = 'dhatu_cache_v3'; 
            const TIME_KEY = 'dhatu_cache_time_v3';
                                            ...
                                            {vocabList.map((v, idx) => (
                                                <li key={idx} style={{ marginBottom: '8px', paddingBottom: '8px', borderBottom: '1px dashed #e0e0e0' }}>
                                                    ...
                                                    <span style={{ color: '#34495e', marginLeft: '5px' }}>
                                                        {v.def.replace(v.word, '').trim().replace(/^:\s*/, '').replace(/สำเร็จรูปเป็น\s*$/, '')} 
                                                    </span>
                                                </li>
                                            ))}
                                        </ul>

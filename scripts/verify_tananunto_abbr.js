...
const filePath = 'd:/pali-theonlyone/data/raw/vocab-tananunto.js';
let fileContent = fs.readFileSync(filePath, 'utf8');

// Hack to make it run in VM: remove 'const ' to make it a global assignment in sandbox
fileContent = fileContent.replace('const vocabTananunto', 'vocabTananunto');

const sandbox = { vocabTananunto: {} };
vm.createContext(sandbox);
vm.runInContext(fileContent, sandbox);
const dictionary = sandbox.vocabTananunto;
...
const abbrList = [
    ...
    { short: 'ทุ.เอก.', full: 'ทุติยาวิภัตติฝ่ายเอกวจนะ' },
    { short: 'ธ.', full: 'ธมฺมปทฏฺกถา หรือธรรมบทบาลีภาค' },
...
console.log(`Scanning ${values.length} entries in vocab-tananunto.js...`);

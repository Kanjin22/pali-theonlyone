...
def main():
    ...
    # Priority: dpd.db.tar.bz2 > dpd.db.zip > dpd.zip
    ...
    # 5. Run scripts
    # These scripts regenerate the JS files used by the frontend
    # Note: Some scripts are in root, some in scripts/ folder
    scripts_to_run = [
        ["python", "extract_dpd.py"],                       # Generates data/vocab-dpd.js
        ["node", "scripts/extract_roots_from_dpd_vocab.js"], # Generates data/vocab-roots-dpd-derived.js
        ["python", "enrich_roots_from_dpd.py"],              # Updates vocab-roots.js with new meanings
        ["node", "scripts/update_roots_firestore.js"]        # Syncs updates to Firestore
    ]

// สร้างโดยเครื่องมือสร้างตารางเรียน (Schedule Builder V2)
var data_pt12_novice_december_2025 = [
    {
        date: "2025-12-01", displayDate: "จ. 01-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๕๘-๕๙",
            fileNoteGrammar: "materials/grammar-notes-58-59.pdf",
            activity: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๕๘-๕๙"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam3-58-59.pdf",
            fileAnswerGrammar: "answers/grammar-answer3-58-59.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๔" },
    },
    {
        date: "2025-12-02", displayDate: "อ. 02-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๐-๖๑",
            fileNoteGrammar: "materials/grammar-notes-60-61.pdf",
            activity: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๐-๖๑"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam3-60-61.pdf",
            fileAnswerGrammar: "answers/grammar-answer3-60-61.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๕" },
    },
    {
        date: "2025-12-03", displayDate: "พ. 03-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๒-๖๓",
            fileNoteGrammar: "materials/grammar-notes-62-63.pdf",
            activity: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๒-๖๓"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam3-62-63.pdf",
            fileAnswerGrammar: "answers/grammar-answer3-62-63.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๖" },
    },
    {
        date: "2025-12-04", displayDate: "พฤ. 04-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๔-๖๕",
            fileNoteGrammar: "materials/grammar-notes-64-65.pdf",
            activity: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๔-๖๕"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam3-64-65.pdf",
            fileAnswerGrammar: "answers/grammar-answer3-64-65.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๗" },
    },
    {
        date: "2025-12-05", displayDate: "ศ. 05-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๖-๖๗",
            fileNoteGrammar: "materials/grammar-notes-66-67.pdf",
            activity: "(อ่าน) ข้อ ๓ (นามศัพท์-อัพยยศัพท์) ปี' ๖๖-๖๗"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam3-66-67.pdf",
            fileAnswerGrammar: "answers/grammar-answer3-66-67.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๘" },
    },
    {
        date: "2025-12-06", displayDate: "ส. 06-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-07", displayDate: "อา. 07-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-08", displayDate: "จ. 08-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๐-๕๑",
            fileNoteGrammar: "materials/grammar-notes-50-51.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๐-๕๑"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-50-51.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-50-51.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๑๙" },
    },
    {
        date: "2025-12-09", displayDate: "อ. 09-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๒-๕๓",
            fileNoteGrammar: "materials/grammar-notes-52-53.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๒-๕๓"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-52-53.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-52-53.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๐" },
    },
    {
        date: "2025-12-10", displayDate: "พ. 10-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๔-๕๕",
            fileNoteGrammar: "materials/grammar-notes-54-55.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๔-๕๕"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-54-55.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-54-55.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๑" },
    },
    {
        date: "2025-12-11", displayDate: "พฤ. 11-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๖-๕๗",
            fileNoteGrammar: "materials/grammar-notes-56-57.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๖-๕๗"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-56-57.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-56-57.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๒" },
    },
    {
        date: "2025-12-12", displayDate: "ศ. 12-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๘-๕๙",
            fileNoteGrammar: "materials/grammar-notes-58-59.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๕๘-๕๙"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-58-59.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-58-59.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๓" },
    },
    {
        date: "2025-12-13", displayDate: "ส. 13-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-14", displayDate: "อา. 14-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-15", displayDate: "จ. 15-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๐-๖๑",
            fileNoteGrammar: "materials/grammar-notes-60-61.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๐-๖๑"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-60-61.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-60-61.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๔" },
    },
    {
        date: "2025-12-16", displayDate: "อ. 16-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๒-๖๓",
            fileNoteGrammar: "materials/grammar-notes-62-63.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๒-๖๓"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-62-63.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-62-63.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๕" },
    },
    {
        date: "2025-12-17", displayDate: "พ. 17-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๔-๖๕",
            fileNoteGrammar: "materials/grammar-notes-64-65.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๔-๖๕"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-64-65.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-64-65.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๖" },
    },
    {
        date: "2025-12-18", displayDate: "พฤ. 18-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๖-๖๗",
            fileNoteGrammar: "materials/grammar-notes-66-67.pdf",
            activity: "(อ่าน) ข้อ ๔-๕ (อาขยาต-กิตก์) ปี' ๖๖-๖๗"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam45-66-67.pdf",
            fileAnswerGrammar: "answers/grammar-answer45-66-67.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๗" },
    },
    {
        date: "2025-12-19", displayDate: "ศ. 19-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๐-๕๑",
            fileNoteGrammar: "materials/grammar-notes-50-51.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๐-๕๑"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-50-51.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-50-51.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๘" },
    },
    {
        date: "2025-12-20", displayDate: "ส. 20-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-21", displayDate: "อา. 21-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-22", displayDate: "จ. 22-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๒-๕๓",
            fileNoteGrammar: "materials/grammar-notes-52-53.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๒-๕๓"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-52-53.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-52-53.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๒๙" },
    },
    {
        date: "2025-12-23", displayDate: "อ. 23-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๔-๕๕",
            fileNoteGrammar: "materials/grammar-notes-54-55.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๔-๕๕"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-54-55.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-54-55.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๐" },
    },
    {
        date: "2025-12-24", displayDate: "พ. 24-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๖-๕๗",
            fileNoteGrammar: "materials/grammar-notes-56-57.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๖-๕๗"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-56-57.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-56-57.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๑" },
    },
    {
        date: "2025-12-25", displayDate: "พฤ. 25-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๘-๕๙",
            fileNoteGrammar: "materials/grammar-notes-58-59.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๕๘-๕๙"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-58-59.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-58-59.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๒" },
    },
    {
        date: "2025-12-26", displayDate: "ศ. 26-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๐-๖๑",
            fileNoteGrammar: "materials/grammar-notes-60-61.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๐-๖๑"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-60-61.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-60-61.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๓" },
    },
    {
        date: "2025-12-27", displayDate: "ส. 27-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-28", displayDate: "อา. 28-ธ.ค.-68",
        isWeekend: true,
    },
    {
        date: "2025-12-29", displayDate: "จ. 29-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๒-๖๓",
            fileNoteGrammar: "materials/grammar-notes-62-63.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๒-๖๓"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-62-63.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-62-63.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๔" },
    },
    {
        date: "2025-12-30", displayDate: "อ. 30-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๔-๖๕",
            fileNoteGrammar: "materials/grammar-notes-64-65.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๔-๖๕"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-64-65.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-64-65.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๕" },
    },
    {
        date: "2025-12-31", displayDate: "พ. 31-ธ.ค.-68",
        morning: {
            activityGrammar: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๖-๖๗",
            fileNoteGrammar: "materials/grammar-notes-66-67.pdf",
            activity: "(อ่าน) ข้อ ๖-๗ (สมาส-ตัทธิต) ปี' ๖๖-๖๗"
        },
        afternoon: {
            activity: "(สอบ)",
            isExam: true,
            examStartTime: "14:00:00",
            examEndTime: "15:00:00",
            fileExamGrammar: "exams/grammar-exam67-66-67.pdf",
            fileAnswerGrammar: "answers/grammar-answer67-66-67.pdf"
        },
        remarks: { general: "บาลีไวยากรณ์ ครั้งที่ ๓๖ (จบ)" },
    },
];
